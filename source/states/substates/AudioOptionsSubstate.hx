...
class AudioOptionsSubstate extends ShiftingMenuSubstate {
    override public function create() {
        super.create();

        var text = new flixel.text.FlxText(0, FlxG.height * 0.1, 0, "Audio (wip)", 64);
        text.screenCenter(X);
        add(text);

        var levelSelectButton = new FlxButton(0, FlxG.height * 0.5, "Master Volume", () -> {
